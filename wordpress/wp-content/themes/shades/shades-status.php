...
<div <?php
    ...
    <div class="postdata">
        ...
        if ( is_home() || is_front_page() ) {
            printf( __( '%1$s by %2$s on %3$s in', 'shades' ), shades_use_posted(), get_the_author(), get_the_time( get_option( 'date_format' ) ) );
            ...
            printf( __( 'Posted by %1$s on %2$s @ %3$s', 'shades' ), get_the_author(), get_the_time( get_option( 'date_format' ) ), get_the_time ( get_option( 'time_format' ) ) );
            _e( '<br />in ', 'shades' ); the_category( ', ' );
        }
