...
function mosaic_remove_wpautop($content) {
    global $post;
...
add_filter('the_content', 'mosaic_remove_wpautop', 9);
...
function mosaic_setup() {
	...
	$locale = get_locale();
	$locale_file = MOSAIC_PARENT_DIR . "/languages/$locale.php";
...
function mosaic_footer() {
	...
	echo '<div id="credits-wrap"><div id="copyright">';
	echo do_shortcode(of_get_option('footer_copyright'));
	echo '</div>';
	
	echo '<div id="credits">';
	echo do_shortcode(of_get_option('footer_credit'));
	echo '</div></div>';
}
...
function mosaic_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'mosaic_theme_options[body_background][color]', array(
		'default'           => $options['body_background']['std'],
		'type'              => 'option',
	...
	$wp_customize->add_setting( 'mosaic_theme_options[general_font_typography][color]', array(
		'default'           => $options['general_font']['std'],
		'type'              => 'option',
		'sanitize_callback' => 'sanitize_hex_color',
		'capability'        => 'edit_theme_options',
	) );
	...
	$wp_customize->add_setting( 'mosaic_theme_options[site_title_typography][color]', array(
		'default'           => $options['site_title']['std'],
		'type'              => 'option',
		'sanitize_callback' => 'sanitize_hex_color',
		'capability'        => 'edit_theme_options',
	) );
	...
	$wp_customize->add_setting( 'mosaic_theme_options[site_tagline_typography][color]', array(
		'default'           => $options['site_tagline']['std'],
		'type'              => 'option',
		'sanitize_callback' => 'sanitize_hex_color',
		'capability'        => 'edit_theme_options',
	) );
	...
	$wp_customize->add_setting( 'mosaic_theme_options[post_title_typography][color]', array(
		'default'           => $options['post_title']['std'],
		'type'              => 'option',
		'sanitize_callback' => 'sanitize_hex_color',
		'capability'        => 'edit_theme_options',
	) );
...
function mosaic_print_style() {
	...
	if ( $post_info_typography = of_get_option('post_info_typography') )
		$style .= '.entry-meta, .entry-meta span, .entry-meta .entry-date a{' . mosaic_generate_font_css($post_info_typography) . '}';
	
	if ( $post_meta_typography = of_get_option('post_meta_typography') )
		$style .= '.entry-utility, .entry-utility span, .entry-utility .entry-date a{' . mosaic_generate_font_css($post_meta_typography) . '}';
